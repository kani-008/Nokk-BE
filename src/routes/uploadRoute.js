...
const { uploadProduct, uploadReview, uploadBannerFile, uploadProductImage, uploadReviewImage, uploadCustomerVideoFile, deleteUploadedFile } = require("../controllers/uploadController.js");
const { authenticate, isAdmin } = require("../middleware/auth.js");

// POST /api/upload/banner  — field: "file", kind: "video"|"image", max 100 MB (multer runs inside handler)
router.post("/banner",  authenticate, isAdmin, uploadBannerFile);

// POST /api/upload/customer-video - field: "file", max 100 MB (multer runs inside handler)
router.post("/customer-video", authenticate, isAdmin, uploadCustomerVideoFile);
