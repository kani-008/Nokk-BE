...
async function getBanners(req, res) {
  console.log({
    route: "GET /api/banners/get-banners",
    status: "fetching active banners",
  });
  try {
    ...
    return res
      .status(200)
      .json({ success: true, banners: result.rows.map(formatBanner) });
  } catch (err) {
    console.error({
      route: "GET /api/banners/get-banners",
      error: err.message,
...
async function getAllBanners(req, res) {
  console.log({
    route: "GET /api/banners/get-all",
    status: "fetching all banners",
  });
  try {
    const result = await db.query(
...
async function createBanner(req, res) {
  ...
  console.log({
    route: "POST /api/banners/create-banner",
    body: { title, subtitle, isActive },
  });
  ...
  try {
    ...
    if (!imageUrl) {
      return res
        .status(400)
        .json({ success: false, message: "imageUrl or imageFile is required" });
    }
    ...
    return res
      ...
      .json({
        ...
        banner: formatBanner(result.rows[0]),
      });
  } catch (err) {
    console.error({
      route: "POST /api/banners/create-banner",
...
async function updateBanner(req, res) {
  ...
  try {
    const current = await db.query(
      "SELECT image_url, video_url FROM banners WHERE id = $1",
      [id],
    );
    ...
    console.log({
      route: "PUT /api/banners/update-banner",
      bannerId: id,
      status: 200,
    });
    return res
...
async function deleteBanner(req, res) {
  ...
  try {
    const result = await db.query(
      "DELETE FROM banners WHERE id = $1 RETURNING image_url, video_url",
      [id],
    );
    if (result.rows.length === 0) {
      return res
    ...
    console.log({
      route: "DELETE /api/banners/delete-banner",
      bannerId: id,
      status: 200,
    });
